...
func WriteToFile(data lex.DataHolder, outDir string) error {
	...
	yamlData, err := yaml.Marshal(map[string]any{data.SheetName(): allLineData})
	if err != nil {
		fmt.Printf("Error while marshaling. %v", err)
		return err
	}

	return writer.WriteToFile(outDir, data.SheetName(), outFileSuffix, yamlData)
}
